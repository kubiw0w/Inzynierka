function noise = perlinNoiseSinDeform(width, height, frequency, marbleFreq, marbleAmp)
    % perlinNoiseSinDeform - Perlin Noise z sinusoidalną deformacją
    %   width, height   - rozmiar tekstury
    %   frequency       - częstotliwość siatki Perlin
    %   marbleFreq      - częstotliwość fali sinusoidalnej
    %   marbleAmp       - amplituda zakłócenia sinusoidalnego
    ...
    % --- Sinusoidalna deformacja ---
    % typowa formuła: sin(x * freq + noise * amp)
    noise = sin(xx/width * marbleFreq*2*pi + nxy * marbleAmp);
